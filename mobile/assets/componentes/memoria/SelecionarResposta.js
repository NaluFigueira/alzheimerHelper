...
export default function SelecionarForma({navigation}){
  ...
  function verificarVitoria(index){
    var aux = []
    for (let i = index+1; i <= index+(nivel*3); i++)
      aux.push(respostasPossiveis[i]);
    let j = 0;
    for (let i = 0; i < aux.length; i++) {
      if(respostaCorreta[j] !== aux[i])
          return false;
      j++;
    }
    return true;
  ...
  function alertarVitoria(){
    Alert.alert(
     ...
     [
       {text: 'CONTINUAR', onPress: () => navigation.push('IdentificarForma',{
         nivel: nivel+1,
         navigation
         }) 
     },
       {
         text: 'VOLTAR',
         onPress: () => this.props.navigation.push('MenuMemoria'),
       },
     ],
     {cancelable: false},
   );
